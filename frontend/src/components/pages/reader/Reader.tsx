...
export const Reader = ({ book }: ReaderProps) => {
  ...
  // Persistir la página actual (local + cloud) cuando cambia. El lector
  // también la usa en vivo para los bookmarks, así que mantenemos el
  // estado local y además sincronizamos a través del store.
  //
  // IMPORTANTE: `onPageChange` se dispara en CADA evento de scroll con el
  // mismo valor repetido, así que sólo persistimos cuando la página real
  // cambió (evita writes innecesarios a IndexedDB y re-renders del store
  // por cada píxel scrolleado).
  const lastPersistedPageRef = useRef<number>(0);
  const handlePageChange = useCallback(
    (page: number) => {
      setCurrentPage(page);
      if (page !== lastPersistedPageRef.current) {
        lastPersistedPageRef.current = page;
        updateCurrentPage(book.id, page);
      }
    },
    [book.id, updateCurrentPage],
  );
